...
click({convert:'101'});assert.equal(vm.runInContext('db.jobs.length',sandbox),7);assert.equal(vm.runInContext('db.leads[0].status',sandbox),'Converted');click({convert:'101'});assert.equal(vm.runInContext('db.jobs.length',sandbox),7);
