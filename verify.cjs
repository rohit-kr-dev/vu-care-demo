...
const node=s=>nodes[s]??={insertAdjacentHTML(){},innerHTML:'',textContent:'',value:'',style:{},dataset:{},classList:{add(){},remove(){},toggle(){}},showModal(){},close(){},focus(){},setSelectionRange(){}};
...
for(const role of ['Admin','Telecaller','Sales','Site Inspector','Operations','Field Staff','Accounts','Support']){vm.runInContext(`role=${JSON.stringify(role)};for(const p of access[role]){page=p;render();if(!$('#main').innerHTML.length)throw Error('Empty page')}`,sandbox)}
